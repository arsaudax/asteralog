  ...
  return (
    <div class={classNames(displayClass, "blog-index")}>
      ...
      <div style={{
        ...
        <h3>📊 Отладка BlogIndex</h3>
        <p><strong>BASE_URL:</strong> {baseUrl || 'не задан'}</p>
        <p><strong>Тип сайта:</strong> {isBlog ? 'БЛОГ' : 'САД'}</p>
        <p><strong>Текущая страница:</strong> {fileData.slug}</p>
        <p><strong>Всего файлов:</strong> {allFiles.length}</p>
        <p><strong>Постов в блоге:</strong> {blogPosts.length}</p>
        <p><strong>Должен рендериться?</strong> {(isBlog && fileData.slug === 'index') ? 'ДА' : 'НЕТ'}</p>
      </div>

      {/* Основной контент (показываем только на главной блога) */}
      {isBlog && fileData.slug === 'index' && (
        <>
            ...
            <div class="author-info">
              <div class="author-avatar-placeholder">
                <svg width="80" height="80" viewBox="0 0 80 80">
                  <circle cx="40" cy="40" r="38" stroke="var(--secondary)" strokeWidth="2" fill="var(--highlight)"/>
                  ...
                  <path d="M60 62C60 54 52 48 40 48C28 48 20 54 20 62" stroke="var(--secondary)" strokeWidth="3" strokeLinecap="round"/>
                </svg>
              </div>
